(function () {
    ...
    $("#btn-salvar-conformidade").click(function (event) {
        conformidade.Id = $("#ConformidadeHiddenId").val();
        conformidade.NumeroConformidade = $("#numero-nao-conformidade").val();
        conformidade.StatusConformidadeId = $('#selectStatus').selectpicker('val'); 
        ...
        $.ajax({
            ...
            success: (data) => {
                if (data.StatusCode == 200) {
                    ...
                    Swal.fire({
                        title: 'Bom trabalho!',
                        html: 'Conformidade editada com sucesso!',
                        icon: 'success',
                        timer: 6000,
                        willClose: () => {
    ...
    function Init() {
        ...
        $("#form-edit-conformidade :input").attr("disabled", "disabled"); 

        $('#btn-salvar-conformidade').removeAttr("disabled", false); 

        
        $('#selectStatus').removeAttr("disabled",false); 
        $('#selectStatus').selectpicker('refresh');
         


        //$("#form-edit-conformidade :button").attr("hidden", "hidden");
